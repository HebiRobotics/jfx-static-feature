 ...
 * Each load calls the initializer directly instead.
 *
...
public final class StaticLibraryLoading {
    ...
    static final class Target_Application {
        ...
        protected static synchronized void loadNativeLibrary(String libname) {
            if (!loaded) {
                WindowsOnLoad.glass(JNIFunctionTables.singleton().getGlobalJavaVM(), WordFactory.nullPointer());
                loaded = true;
            }
        }
    ...
    static void initialize(String libname) {
        ...
        int version = Platform.includedIn(Platform.LINUX.class)
                ? LinuxOnLoad.onLoad(libname, vm, reserved)
                : MacOnLoad.onLoad(libname, vm, reserved);
    ...
    private static final class WindowsOnLoad {
        ...
        static native int glass(JNIJavaVM vm, PointerBase reserved);
