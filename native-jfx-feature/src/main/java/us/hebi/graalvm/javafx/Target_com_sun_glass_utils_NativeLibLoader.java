 ...
 * hides every symbol the image does not export itself behind a linker version script on Linux and
 * an exported symbols list on macOS, so no JavaFX library can be loaded that way. Each load is
 * therefore replaced by a direct call to the library's initializer, which the linker resolves. This
 * also drops the JNI version handshake, which glass fails on both platforms because it reports
 * JNI_VERSION_1_6 (Linux) or 1_4 (macOS) instead of the 1.8 the specification requires from a
...
final class StaticJavaFXLibraries {
    ...
    static void initialize(String libname) {
        ...
        int version = Platform.includedIn(Platform.LINUX.class)
                ? LinuxLibraries.onLoad(libname, vm, reserved)
                : MacLibraries.onLoad(libname, vm, reserved);
