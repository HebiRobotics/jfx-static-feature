...
/**
 * glass posts its run loop to the first thread of the process and waits for it, so that thread has
 * to be inside a CFRunLoop while FX starts. The java launcher parks it there (ParkEventLoop in
 * java_md_macosx.m) and runs main on a second thread, a native image runs main on the first thread
 * and hangs in glass' "waiting for [glass started]" loop. Same method as upstream with the latch
 * wait replaced by the run loop, and only when called on the first thread: a launcher that already
 * owns Cocoa calls launch from a background thread, which has no run loop to pump.
 *
...
final class Target_com_sun_javafx_application_LauncherImpl {
